...
4. 产出 Mermaid/Markdown 总览图、模块图、测试矩阵和评审问题清单。
5. 给出评审建议，明确哪些内容已满足进入 API/UI/Load 生成阶段，哪些内容仍需补充。
...
默认输出到 `output/automation-factory/{yyMMdd}_{requirement_slug}/test_cases/`。
...
- `module-01-<slug>.md` 到 `module-06-<slug>.md`
  使用 `assets/module-map-template.md`，每个文件只覆盖一个业务模块，输出模块级 Mermaid 图、场景覆盖说明、关键前置条件。
- `test-matrix.md`
  使用 `assets/test-matrix-template.md`，输出模块、场景、用例类型、前置条件、预期结果、自动化映射的测试矩阵表。
- `review-issues.md`
  使用 `assets/review-issues-template.md`，输出待澄清项、矛盾项、评审动作。

不要默认把所有内容塞进一张 Mermaid 里；只有需求极小、模块不超过 2 个时，才允许减少模块图数量。
同一需求的下游 API/UI/Load Creator skill 必须复用同一个 `{yyMMdd}_{requirement_slug}` 根目录，只切换到各自子目录。
...
- 完整覆盖关系放在 `test-matrix.md`，不要试图用一张图表达所有信息。
...
- 如果图能看懂但无法追踪到具体用例，说明 `test-matrix.md` 不完整。
...
- 使用 `assets/mindmap-template.md`、`assets/module-map-template.md`、`assets/test-matrix-template.md` 和 `assets/review-issues-template.md` 作为默认产物骨架。
